...
def feistelEncrypt(mensaje,llaves,ciclos):
    ...
    mensaje = R + exor(F,L,mitad) # R + L EXOR F
     #Si quedan cifrados por hacer vuelvo a llamar a la funcion
        
    if ciclos > 1:
        return feistelEncrypt(mensaje,llaves,ciclos-1)
    else:
...
# Se transforman los caracteres a numeros
palabra = [ord(i) for i in palabra] 
# Se transforman los numeros a binario
palabra = [format(i,"08b") for i in palabra] 
# Se une la lista de binarios en un string sin espacios
palabra = "".join(palabra) 
# Se calcula la mitad del string, para separar las mitades
mitad = len(palabra)//2 
